...
//     result
  ...
  def listmykey(String bookInitial)  {
  ...
  def BooknamePair() {
/*
	BibleBook.values().each{
	println it.getShortName()+"/"+it.getOSIS()+"/"+it.getPreferredName()
	}
*/
		
    for (bn in 1..66) {
        def book_short = BibleInfo.getShortBookName(bn)
        def book = BibleInfo.getBookName(bn)
	println book.toString()+"/"+book_short.toString()
      }
}
  def randomVerse() {
  ...
  private SAXEventProvider getOSISProvider(String bookInitials, String reference, int start, int count) throws BookException, NoSuchKeyException {
	println "reference in getOSISProvider:"+reference
  ...
  public int getCardinality(String bookInitials, String reference) throws NoSuchKeyException {
	...
	println "reference to do:"+reference
      if(reference?.startsWith("1 ")||reference?.startsWith("2 ")|| reference?.startsWith("3 ")) reference=reference.replacefirst(" ","")
    Book book = BookInstaller.getInstalledBook(bookInitials);
    if (book != null) {
  ...
  private BookData getBookData(String bookstring, String reference, int start, int count) throws NoSuchKeyException {
	println "getBook Data bookstring:"+bookstring +" ref:"+reference
  ...
  def getBookKey(Book book, String reference, int start, int count) throws NoSuchKeyException {
    ...
    Key key = null;
    if (BookCategory.BIBLE.equals(book.getBookCategory())) {
	println ("getBookkey ref:"+reference +":bookname:"+book.name)
      key = book?.getKey(reference);
