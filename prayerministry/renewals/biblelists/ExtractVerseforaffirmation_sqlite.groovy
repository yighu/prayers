...
import org.crosswire.jsword.book.*
  ...
  def findVersion(List books, String version) {
    def rst
    books.each {book ->
  ...
  def findBible(List bibles, String name) {
    def rst
...
//	println "bookInitials:"+bookInitials
   try{
    ...
    println ("In Read Styled Text bookInitials:"+bookInitials +" ref:"+ reference)
...
def idex=1
def versionobj=  findVersion(books, version)
	println "version:"+versionobj +" ref:"+ref
            def searchresult = searchByRef(versionobj?.initials?: "ChiUns", ref)
...
//source?.replaceAll("</w:t>","").replaceAll("</w:r>","").replaceAll("<w:r>","").replaceAll("<w:rPr>","").replaceAll("</w:rPr>","").replaceAll("""<w:rFonts w:hint="eastAsia"/>""","").replaceAll("<w:t>","").replaceAll("""<w:t xml:space="preserve">""","")
      ...
      source?.replaceAll(/\<(.*?)\>/,"")?.replaceAll("\n"," ").replaceAll("\r"," ").replaceAll("Revelation","Rev").replaceAll("Isaiah","Isa").replaceAll("Matthew","Mat").replaceAll("Corinthians","Cor")
}
...
try{
def writer = new File(destination).newWriter("UTF-8", true)
...
sources.each{source->
try{
translateBible(source+".htm",source+"_ChiUnt.html","ChiNCVt")
...
def dest="/Users/yiguanghu/ccim/prayerministry/dailyhtml/day"
(2..2).each{day->
translateBible(sourceroot+day+".htm",dest+day+".html","ChiNCVt")
}
...
//splittodaily("/Users/yiguanghu/ccim/prayerministry/daily/header.htm","/Users/yiguanghu/ccim/prayerministry/daily/tail.htm","/Users/yiguanghu/ccim/prayerministry/daily/Skeleton_ChiNCVt.htm","/Users/yiguanghu/ccim/prayerministry/daily/tmp/")
    ...
    //updateBibles(Locale.US)
    //updateBibles(Locale.CHINA)
  def getBiblenames(Locale loc) {

      int nbook = BibleInfo.booksInBible();

      BibleNames bn = new BibleNames(loc)
def writer= new File("out_e.txt").newWriter("UTF-8",true)
def rst=new StringBuilder()
  def mp=new HashMap()
          def gb = new Expando()
      try {
        for (int i = 1; i <= nbook; i++)

        {
          ...
          gb.cname = bn.getName(i)
          gb.longname = BibleInfo.getBookName(i).getLongName()
	rst<<gb.key+","+gb.shortname+","+gb.cname+","+gb.longname+";\n"
	mp.put(gb.longname,gb.shortname)
        }
      } catch (Exception e) {
        e.printStackTrace();
    }
...
def replaceNames(txt,mp){
	mp?.each{key,value->
	txt=txt?.replaceAll(key,value)
	}
txt
}
//println "final:"+	fetchVerses("KJV",cleanText("Revelation 5:13"))
//println "final:"+	fetchVerses("KJV",cleanText("Rev 5:13"))

//System.exit(0)
// def mp= getBiblenames(Locale.CHINA ) 
 def mp= getBiblenames(Locale.US) 
System.exit(0)
def version="ChiNCVt"
version="KJV"
...
try{
println "insert..."+txt+" ver:"+version+" reus:"+result+" cat:"+folder
p.insert([txt,version,result,folder])
/*
def writer= new File(dest+folder+"/s"+i+++".txt").newWriter("UTF-8",true)
txt=replaceNames(txt,mp)//Do this for chinese bible only
writer.write(result.toString()+" ($txt)"); 
writer.close()
*/
}catch(Exception e){}
        }
